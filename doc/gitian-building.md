...
*Setup instructions for a gitian build of BanKitt using a Debian VM or physical system.*
...
- [Building BanKitt](#building-bankitz)
...
Clone the git repositories for bankitz and gitian and then checkout the bankitz version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/modcrypto/bankitz.git
cd bankitz
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bankitz repository
...
To build BanKitt (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bankitz repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bankitz/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/modcrypto/bankitz
...
For example:
```bash
URL=https://github.com/modcrypto/bankitz
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bankitz=${COMMIT} --url bankitz=${URL} ../bankitz/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bankitz=${COMMIT} --url bankitz=${URL} ../bankitz/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bankitz=${COMMIT} --url bankitz=${URL} ../bankitz/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bankitz-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bankitz-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bankitz-build.assert
```
...
[bankitz/gitian.sigs](https://github.com/bankitz-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
