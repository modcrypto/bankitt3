...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (BKZ users should be well used to small numbers with many decimal places by now :)
                ...
                if (lockRecv) {
                    {
                        ...
                        if (nBytes > 0) {
                            ...
                            if (nErr != WSAEWOULDBLOCK && nErr != WSAEMSGSIZE && nErr != WSAEINTR && nErr != WSAEINPROGRESS) {
                                if (!pnode->fDisconnect)
                                    LogPrintf("socket recv error %s %s %s\n",pnode->addrName, pnode->addr.ToString(),  NetworkErrorString(nErr));
                                pnode->CloseSocketDisconnect();
                            }
                        }
                    }
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "BKZ " + FormatFullVersion();
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (!addr.IsValid() || setConnected.count(addr.GetGroup()) || IsLocal(addr)){
              /* std::string c="";
               if(!addr.IsValid()) c=c+" !isValid "; 
               if(setConnected.count(addr.GetGroup())) c=c+" Connected "; 
               if(IsLocal(addr)) c=c+" IsLocal "; 
               LogPrintf("ThreadOpenConnections skip %s %s\n", addr.ToString(),c);
               */
               break;
            }
...
{
    // LogPrintf("OpenNetworkConnection to %s \n", addrConnect.ToString());
    //
    ...
    if (!pszDest) {
        if (IsLocal(addrConnect) ||
          ...
          //  LogPrintf("OpenNetworkConnection  cannot connect \n");
            return false;
        }
    } else if (FindNode(pszDest)){
        // LogPrintf("OpenNetworkConnection  cannot connect found Node \n");
        return false;
    }
    ...
    if (!pnode){
        //LogPrintf("OpenNetworkConnection  cannot connect !pnode \n");
        return false;
    }
    if (grantOutbound)
    ...
    LogPrintf("OpenNetworkConnection   OK \n");
    return true;
}
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BanKitz Core is probably already running."), addrBind.ToString());
        else
