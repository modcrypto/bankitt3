...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BKZ)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BKZ")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeBKZAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBKZAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBKZAmount = strAnonymizeBKZAmount.remove(strAnonymizeBKZAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBKZAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBKZAmount * COIN) nMaxToAnonymize = nAnonymizeBKZAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBKZAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBKZAmount));
        strAnonymizeBKZAmount = strAnonymizeBKZAmount.remove(strAnonymizeBKZAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBKZAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::updateInformation(){
    ...
    QString txt = tr("<h2>BanKitz Information</h2>\n"); 
    if(theme=="dark")
