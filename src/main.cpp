...
#if defined(NDEBUG)
#error "BanKitz cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BanKitz Signed Message:\n";
...
{    
    ...
    if(nHeight<=LastPOWBlock){ // lastpowblock
      for(int n=block_per_month; n < nHeight; n+= block_per_month){
         nSubsidy -= 10*COIN;
      } 
      if(nSubsidy < 10*COIN) nSubsidy = 10*COIN;
    }else{
       nSubsidy = 20 * COIN; 
       for(int n=block_per_month+LastPOWBlock; n < nHeight; n+= block_per_month){
         nSubsidy -= (0.1*nSubsidy);
       } 
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, CAmount nMasternodeCoin)
{
    ...
    if(nHeight<=LastPOWBlock){  
      ...
      if(difficulty < 1) ret = blockValue/100*60; 
      else if(difficulty < 2)  ret = blockValue/100*55;
      else if(difficulty < 3)  ret = blockValue/100*50;
      else if(difficulty < 4)  ret = blockValue/100*45;
      else ret = blockValue/100*40;
    }
    if(nHeight>LastPOWBlock){
      ret = blockValue/2;
    }
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_work=%.8g  tx=%lu  date=%s progress=%f  cache=%u time=%u\n",
