...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
	...
	txNew.vout[0].nValue = GetBlockValue(prev->nHeight, prev->nTime);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BKZMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BKZMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BKZMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bankitz-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("BKZMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running BKZMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadBKZMiner() exception");
    } catch (...) {
        LogPrintf("ThreadBKZMiner() exception");
    }

    LogPrintf("ThreadBKZMiner exiting\n");
}
